...
class PerformanceEnrichmentAnalysisClassifier(BaseEstimator, ClassifierMixin):
...
#             X = pd.DataFrame(X)
#         df_predicted = self._pea_predict(X)
#         classes = np.unique(df_predicted.values)
#         total = len(df_predicted.values)
#         print(df_predicted.values/total)
# #         for c in classes:
# #             n_examples = len(df_predicted.values[df_predicted.values==c])
# #             percent = n_examples / total * 100
# #             print('> Class=%d : %d/%d (%.1f%%)' % (c, n_examples, total, percent))
#         return df_predicted.values/total
